...
const initApp = () => {
  ...
  if (isProd || isCloudRun) {
    console.info(
      'Cloud environment detected. Using default Firebase credentials.',
    );
    try {
      ...
      console.error(
        'Default Firebase initialization failed in cloud environment:',
        error,
      );
  ...
  console.info(
    'Using service account credentials for Firebase initialization.',
  );

  const serviceAccount = getServiceAccount();
  if (!serviceAccount) {
    throw new Error(
      'Firebase initialization failed: Missing required environment variables (FIREBASE_PROJECT_ID, FIREBASE_PRIVATE_KEY, FIREBASE_CLIENT_EMAIL)',
    );
  }
